...
A parameter file is used to pass values to simulations. The [example file](parameters/fluctuating/group_x.txt) can be used with the explanations in quotations removed. We recommend saving the parameter files as ['group_x.txt'](group_x.txt), where x is the group number (unique identifier), to the [parameter folder](parameters) and in a folder labelled with the selection type (i.e. [fluctuating](parameters/fluctuating). The code is structured to extract parameter values from the files in this folder and labelled in this manner. 
