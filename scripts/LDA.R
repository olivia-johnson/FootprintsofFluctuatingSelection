...
train.data <- data[ n_win==250 & gen_year==10& run>150]
test.data <- data[ n_win==250 & gen_year==10&run<=150]
...
formulaStepwise=sim_type ~ H2H1 + diversity  + H1 + H123 + variance
...
train.data <- data[ n_win==250 & (gen_year==10 |gen_year==0) & run>150] ## subset training dataset using sampled from end of seasons
train.data=dcast(train.data, sel_type+sim_type+year+run+label ~ gen_year, value.var = stats) ## transform dataset for LDA
test.data <- data[ n_win==250 & (gen_year==10 |gen_year==0)&run<=150] # subset test dataset using sampled from end of seasons
test.data=dcast(test.data, sel_type+sim_type+year+run+label ~ gen_year, value.var = stats) ## transform dataset for LDA

# Number of categories
numberofCat=length(unique(train.data$sim_type))

# Formual of all statistics to be used for step-wise grouping
...
# Copy formula from output of above function
formulaStepwise=sim_type ~ H2H1_0 + diversity_10 + H1_0 + H123_0 + variance_10 + diversity_0 + 
  H2H1_10 + variance_0 + tajimas_d_site_0 + skew_10
...
train.data <- data[ n_win==250 & (gen_year==10 |gen_year==5)& run>150 ]
train.data=dcast(train.data, sel_type+sim_type+year+run+label ~ gen_year, value.var = stats)
test.data <- data[ n_win==250 & (gen_year==10 |gen_year==5)&run<=150]
...
formulaStepwise=sim_type ~ H2H1_5 + diversity_5 + variance_10 + H1_5 + H123_5 + H2H1_10 + diversity_10 +
   H1_10 + H123_10 + tajimas_d_site_5 + skew_5
...
train.data <- data[ (n_win==250|n_win==275|n_win==251) & gen_year==10& run>150]
train.data=dcast(train.data, sel_type+sim_type+year+run+label ~ n_win, value.var = stats)
test.data <- data[ (n_win==250|n_win==275|n_win==251) & gen_year==10 &run<=150]
...
formulaStepwise=sim_type ~ H2H1_250 + diversity_250 + H1_250 + 
  H123_250 + variance_250 + diversity_251 + variance_251
...
ggsave("plots/S16.pdf", s01, width = 15, height = 10)
...
formulaStepwise=sim_type ~ H2H1 + variance + diversity + H1 + H123 + ncd_5 + ncd4_z + H12 +
  skew + kurtosis + theta_w_allele + ncd_4 + ncd_3
...
## end of seasons
train.data <- data[ n_win==250 & (gen_year==10 |gen_year==0)& run>150]
train.data=dcast(train.data, sel_type+sim_type+year+run+label ~ gen_year, value.var = stats)
test.data <- data[ n_win==250 & (gen_year==10 |gen_year==0)&run<=150]
test.data=test.data=dcast(test.data, sel_type+sim_type+year+run+label ~ gen_year, value.var = stats)

numberofCat=length(unique(train.data$sim_type))

formulaAll=sim_type~diversity_0+tajimas_d_site_0+theta_w_allele_0+ncd_4_0+ncd_3_0+ ncd_5_0 +ncd5_z_0+ncd4_z_0+ncd3_z_0+
  variance_0+skew_0+kurtosis_0+H1_0+H12_0+H123_0+H2H1_0+diversity_10+tajimas_d_site_10+theta_w_allele_10+ncd_5_10+ncd_4_10+
  ncd_3_10+ncd5_z_10+ncd4_z_10+ncd3_z_10+variance_10+skew_10+kurtosis_10+H1_10+H12_10+H123_10+H2H1_10

greedy.wilks(formulaAll,data=train.data, niveau = 0.05) 

formulaStepwise=sim_type ~ H2H1_10 + H2H1_0 + variance_10 + variance_0 + theta_w_allele_10 + 
  H1_10 + tajimas_d_site_0 + tajimas_d_site_10 + H123_10  + theta_w_allele_0 + ncd_3_10 + ncd_3_0 + ncd3_z_0 + 
  H12_10 +  ncd3_z_10 + diversity_0 + ncd4_z_10 + diversity_10
...
formulaStepwise=sim_type ~ H2H1_10 + variance_10 + variance_5 + theta_w_allele_10 + ncd_3_10 +
  H1_10 + H123_10 + ncd_4_5 + H2H1_5 + ncd_5_10 +  diversity_5 + theta_w_allele_5 + ncd_3_5 + H12_10 + diversity_10 + 
 tajimas_d_site_5+ tajimas_d_site_10 
...
formulaStepwise=sim_type ~ H2H1_250 + variance_250 + diversity_250 + theta_w_allele_251 + H1_250 + H123_250 +
  theta_w_allele_275 + ncd_5_250 + H2H1_251 + H12_250 + ncd4_z_250 + skew_250 + kurtosis_250 + ncd_4_250 +
   ncd3_z_250 + H123_251 + H12_251 + tajimas_d_site_275  + theta_w_allele_250 + ncd5_z_250   
...
ggsave("plots/Fig7.pdf", s05, width = 15, height = 10)
...
formulaAll=sim_type~diversity+tajimas_d_site+theta_w_allele+ncd_5+ncd_4+ncd_3+ncd5_z+ncd4_z+ncd3_z+variance+skew+kurtosis+H1+H2H1+ H12 + H123

greedy.wilks(formulaAll,data=train.data, niveau = 0.05) 

formulaStepwise=sim_type ~ H123 + H2H1 + theta_w_allele + H12 + tajimas_d_site + variance + H1 +
  diversity + ncd_4 + ncd_5 + skew + kurtosis + ncd_3 
...
formulaStepwise=sim_type ~ H123_10 + H2H1_0 + theta_w_allele_10 + H2H1_10 + H12_10 + ncd_3_0 +
  theta_w_allele_0 +  ncd_4_10 + ncd4_z_0 + diversity_10 + variance_0 + H1_10 + H1_0 + H123_0 + ncd3_z_10 + tajimas_d_site_0 + 
  H12_0 + kurtosis_10 + kurtosis_0 + diversity_0 
...
formulaAll=sim_type~diversity_5+tajimas_d_site_5+theta_w_allele_5+ncd_5_5+ncd_4_5+ncd_3_5+ ncd5_z_5+ncd4_z_5+ncd3_z_5+ ncd5_z_10+ncd4_z_10+ncd3_z_10+
  variance_5+skew_5+kurtosis_5+H1_5+H12_5+H123_5+H2H1_5+diversity_10+tajimas_d_site_10+theta_w_allele_10+ncd_5_10+ncd_4_10+
  ncd_3_10+variance_10+skew_10+kurtosis_10+H1_10+H12_10+H123_10+H2H1_10

greedy.wilks(formulaAll,data=train.data, niveau = 0.05) 

formulaStepwise=sim_type ~  H123_10 + H2H1_5 + theta_w_allele_10 + H12_10 + tajimas_d_site_10 + diversity_5 + 
  ncd5_z_10 +  H123_5 + H1_5 + ncd_5_10 + kurtosis_10 + H2H1_10 + kurtosis_5 + diversity_10 +  ncd_3_5
...
formulaAll=sim_type~diversity_250+tajimas_d_site_250+theta_w_allele_250+ncd_5_250+ncd_4_250+ncd_3_250+ncd5_z_250+ncd4_z_250+ncd3_z_250+
  variance_250+skew_250+kurtosis_250+H1_250+H12_250+H123_250+H2H1_250+diversity_275+tajimas_d_site_275+theta_w_allele_275+ncd_5_275+ncd_4_275+
  ncd_3_275+ncd5_z_275+ncd4_z_275+ncd3_z_275+variance_275+skew_275+kurtosis_275+H1_275+H12_275+H123_275+H2H1_275+diversity_251+
  tajimas_d_site_251+theta_w_allele_251+ncd_4_251+ncd_5_251+
  ncd_3_251+ncd5_z_251+ncd4_z_251+ncd3_z_251+variance_251+skew_251+kurtosis_251+H1_251+H12_251+H123_251+H2H1_251

greedy.wilks(formulaAll,data=train.data, niveau = 0.05) 

formulaStepwise=sim_type ~ H123_250 + H2H1_250 + theta_w_allele_250 + theta_w_allele_251 + H12_250 +
   theta_w_allele_275 + tajimas_d_site_250 + variance_250 + H1_250 +H2H1_251 + ncd3_z_250 + ncd_3_250 +
   diversity_250  + ncd_5_250 + skew_250 + kurtosis_250 + ncd_4_250 + H1_275 + H123_251 + H1_251 + ncd_3_251 + 
  kurtosis_251 
...
ggsave("plots/FigS17.pdf", s1, width = 15, height = 10)
